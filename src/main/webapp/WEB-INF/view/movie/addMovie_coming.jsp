...
<body>
	...
	<section id="addMovie">
        <div class="container">
            <h1>영화등록 <span>(상영예정작)</span></h1>
            <hr>
            <div class="form_box">
                <form action="procAddMovie_coming" method="post" name="addMovieForm" enctype="multipart/form-data">
                    ...
                    <div class="form_group">
                        <label for="openDate" class="label">개봉 예정일</label>
    ...
    <!-- 개봉일을 오늘 날짜 이후로 클릭하게 하기 -->
    <script type="text/javascript">
	    let now = new Date();
	    let plus7 = new Date(now.setDate(now.getDate() + 2));
		// getTimezoneOffset()은 현재 시간과의 차이를 분 단위로 반환
		let timeOff = new Date().getTimezoneOffset()*60000; // 분단위를 밀리초로 변환
		
		let nextWeek = new Date(plus7-timeOff).toISOString().split("T")[0];
		console.log(nextWeek);
		
		document.getElementById("openDate").setAttribute("min", nextWeek);
    </script>
