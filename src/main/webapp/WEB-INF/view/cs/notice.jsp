...
<body>
	...
	<section id="section02">
        <div class="container">
            ...
            <div id="notice" class="tap_box view">
            	<div class="search">
		        	<form action="notice" method="post">
						<select name="items_notice">
							<option value="title">제목</option>
						</select> 
						<input name="text_notice" type="text" placeholder="검색어를 입력해주세요." size="50"> 
                    ...
                    <tbody>
                        ...
                        <c:forEach var="noticeList" items="${noticeList}" varStatus="status">
	                        <tr>
	                            ...
	                            <td align="left">
	                            	<a href="noticeView?num=${noticeList.num}">
	                            		${noticeList.title}
	                            	</a>
	                            </td>
	                            <td>${noticeList.regist_day}</td>
	                        </tr>
	                   	</c:forEach>
	                   	<c:choose>
                ...
                <br><br><br>
                <c:set var="pageNum_notice" value="${pageNum_notice}" />
				<c:forEach var="i" begin="1" end="${total_page_notice}">
					<a href="<c:url value="/notice?pageNum_notice=${i}" /> ">
						<c:choose>
							<c:when test="${pageNum_notice==i}">
								<b> [${i}]</b>
							</c:when>
				...
				<%
                	if(admincheck == "true"){
				%>
                	<a href="addNotice" class="button">등록하기</a>
                <%	} %>
            </div>
        </div>
    </section>
